...
function supprimerAdherent(codeAdherent) {
    ...
    if (!adherent) {
      ...
      messageErreur.textContent = "Aucun adhérent ne correspond à ces informations.";
    ...
    if (suppForm) {
      if (suppForm.style.display === "block") {
        ...
        const supprimerBtn = suppForm.querySelector(`#supprimerBtn-${codeAdherent}`);
        if (supprimerBtn) {
      ...
      form.querySelector(`#supprimerBtn-${codeAdherent}`).addEventListener("click", function () {
        const nom = document.querySelector(`#supprimerNom-${codeAdherent}`).value;
        const prenom = document.querySelector(`#supprimerPrenom-${codeAdherent}`).value;
        const email = document.querySelector(`#supprimerEmail-${codeAdherent}`).value;
        const messageErreur = document.getElementById("messageErreur");
  
        if (nom !== adherent.nom || prenom !== adherent.prenom || email !== adherent.email) {
          messageErreur.textContent = "Les informations ne correspondent pas à l'adhérent sélectionné.";
          clearMessage(messageErreur);
          return;
        }
